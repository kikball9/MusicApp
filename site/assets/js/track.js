...
// Affichage dynamique des informations d'un morceau dans la section Morceau
function displayTrackInfos(track){

    document.getElementById("artist-page-album-container").innerHTML = "";

    for (var i =0;i<track.length;i++){
        // affiche les informations du morceau souhaité
...
// Affichage dynamique des informations d'un artiste dans la section Morceau
    ...
    for(var i=0; i<artists.length; i++){
        for(var j=0; j<artists[i]["artist-albums"].length; j++){
            for(var k=0; k<artists[i]["artist-albums"][j]["album-tracks"].length; k++){
                ...
                // Si le morceau appartenant à un artiste correspond au morceau souhaité
                if(artists[i]["artist-albums"][j]["album-tracks"][k]["id_tracks"] == idTrack){
                    ...
                    // remplissage des informations de l'artiste
...
// Affichage dynamique des informations d'un album dans la section Morceau
    ...
    for(var i=0; i<myAlbum["artist-albums"].length; i++){
        for(var j=0; j<myAlbum["artist-albums"][i]["album-tracks"].length; j++){
            ...
            if(myAlbum["artist-albums"][i]["album-tracks"][j]["id_tracks"] == idTrack){
                ...
                // remplissage des informations de l'album
...
// Affichage de la section Morceau
...
/* Gestion des boutons dans les morceaux */
...
/* Bouton scroll options dans le bouton info d'un morceau */
